...
Current state: **M0–M20 closed**; latest release **v0.8.5** (2026-09-24), which adds FlupCode's "Code" colour palette to Settings → Appearance (OG-111). Before that, v0.8.4 (2026-09-23) added FlupCode-derived color palettes (OG-110). Before that, v0.8.3 (2026-09-22) ran blocking git commands off the main thread to fix the Linux tab-switch freeze (OG-109, ADR-0010). Before that, v0.8.2 (2026-09-22) added inline renaming of the session tabs (OG-108). Before that, v0.8.1 (2026-09-22) added drag-and-drop reordering of the session tabs (OG-107). Before that, v0.8.0 (2026-09-21) shipped full Git LFS (track/pull/migrate, OG-097), a hooks manager (OG-098), a typed `en`/`es` i18n catalog with the language selector (OG-099/OG-100, ADR-0009), the migration of every view (OG-102..OG-105) and the native menu (OG-106). Before that, v0.7.0 (2026-09-20) shipped the repository lifecycle: clone and create from the home screen (OG-085..OG-087), windows per repository (OG-088, ADR-0008), recovery: reflog/undo, bisect, reset modes and reverting merges (OG-089..OG-092) and history depth: worktree search, patches, word diff and cherry-pick ranges (OG-093..OG-096). There is no active milestone; scope for the next one starts from `ROADMAP.md`. In-app auto-updates use a free minisign key (OG-081, ADR-0007); signing and notarization are dropped for cost (OG-028).
