...
Current state: **M0–M20 closed**; latest release **v0.7.0** (2026-09-20), which adds the repository lifecycle: clone and create from the home screen (OG-085..OG-087), windows per repository (OG-088, ADR-0008), recovery: reflog/undo, bisect, reset modes and reverting merges (OG-089..OG-092) and history depth: worktree search, patches, word diff and cherry-pick ranges (OG-093..OG-096). **M19** added full Git LFS (track/pull/migrate, OG-097) and a hooks manager (OG-098). **M20 (Internationalization)** added a typed `en`/`es` catalog (OG-099, ADR-0009), the language selector in Settings (OG-100), the migration of every view (OG-102..OG-105) and the native menu (OG-106). There is no active milestone; scope for the next one starts from `ROADMAP.md`. In-app auto-updates use a free minisign key (OG-081, ADR-0007); signing and notarization are dropped for cost (OG-028).
