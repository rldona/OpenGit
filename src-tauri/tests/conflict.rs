...
fn conflicted_repo() -> TestRepo {
    ...
    assert!(!merge.status.success(), "debe quedar en conflicto");
    repo
}

#[test]
fn lee_el_fichero_con_marcadores_y_lo_resuelve() {
    ...
    let (content, binary) = read_worktree_file(repo.path(), "a.txt").expect("leer conflicto");
    assert!(!binary);
...
#[test]
fn valida_rutas_y_detecta_binarios() {
    let repo = conflicted_repo();

    let error = read_worktree_file(repo.path(), "../fuera.txt").expect_err("ruta fuera");
    assert!(
        format!("{error}").contains("path outside the repository"),
        "{error}"
    );

    let error = write_and_stage(&runner(), repo.path(), "/etc/hosts", "x").expect_err("absoluta");
    assert!(
        format!("{error}").contains("path outside the repository"),
        "{error}"
    );
    ...
    let (content, binary) = read_worktree_file(bin.path(), "bin.dat").expect("leer binario");
    assert!(binary);
